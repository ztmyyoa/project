...
const filteredas = computed(() => {
  ...
  return as.value.filter(a =>
      a.advertid?.toString().includes(keyword) ||
...
const fetcha = async () => {
  try {
    const res = await fetch('http://localhost:8080/spring_mvc_mysql_vue_war/ad/all')
...
const handleSubmit = async () => {
  try {
    if (dialogTitle.value === '新增广告') {
      console.log('新增广告:', form.value)
      const res = await axios.post('http://localhost:8080/spring_mvc_mysql_vue_war/ad/add', form.value)
      ...
      console.log('编辑广告:', form.value)
      const res = await axios.put('http://localhost:8080/spring_mvc_mysql_vue_war/ad/update', form.value)
      if (res.data) {
...
const handleDelete =async (id) => {
  try {
    console.log('删除广告:', id)
    const res = await axios.delete(`http://localhost:8080/spring_mvc_mysql_vue_war/ad/delete/${id}`)
    console.log('返回的数据:', res.data)
    if (id) {
      ElMessage.success('删除成功')
      as.value = as.value.filter(f => f.advertid !== id)
    } else {
...
<template>
  <div class="page-container">
    <div class="toolbar">
      <el-button type="primary" @click="openDialog('create')">新增广告</el-button>
      <el-input
    ...
    <el-table :data="pagedas" border stripe style="width: 100%">
      <el-table-column prop="advertid" label="ID" width="80" />
      <el-table-column prop="title" label="广告标题" />
      <el-table-column prop="adcontent" label="内容" />
      <el-table-column prop="imageUrl" label="图片链接" />
      <el-table-column prop="linkUrl" label="网页链接" />
      <el-table-column prop="startTime" label="开始时间" />
      <el-table-column prop="endTime" label="结束时间" />
      <el-table-column label="操作" width="200">
        <template #default="scope">
          <el-button size="small" type="primary" @click="openDialog('edit', scope.row)">编辑</el-button>
          <el-button size="small" type="danger" @click="handleDelete(scope.row.advertid)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="500px">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="内容">
          <el-input v-model="form.adcontent" />
        </el-form-item>
